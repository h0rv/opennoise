...
from musix.serving.local.local_reviewed_alias_context_store import (
...
class LocalReviewedAliasContextStoreTests(unittest.TestCase):
    ...
    def test_startup_verifies_once_and_preserves_context_provenance(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "musix.serving.local.local_reviewed_alias_context_store.ReviewedAliasContextArtifact.model_validate_json",
                    return_value=_artifact(),
                ),
                patch(
                    "musix.serving.local.local_reviewed_alias_context_store.ReviewedAliasCombinedModelReceipt.model_validate_json",
                    return_value=_receipt(),
                ),
                patch(
                    "musix.serving.local.local_reviewed_alias_context_store."
    ...
    def test_startup_rejects_an_invalid_receipt_binding(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch(
                    "musix.serving.local.local_reviewed_alias_context_store.verify_reviewed_alias_context_artifact"
                ),
                patch(
                    "musix.serving.local.local_reviewed_alias_context_store.verify_reviewed_alias_combined_model_receipt"
                ),
