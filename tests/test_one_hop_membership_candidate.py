...
def _inputs(*, support: int = 15, hierarchy: bool = True) -> PublicModelInput:
    return PublicModelInput(
        artifacts=(
            PublicArtifact(
                source="listenbrainz", snapshot="fixture", artifact_key="fixture", content_sha256="a" * 64, export_allowed=True
            ),
        ),
        genres=(
        ...
        direct_memberships=(
            DirectMembershipEvidence(artist_id="artist:one", genre_id="genre:a", facet="musicbrainz_tag", value=10, evidence_ref="direct:one:a"),
            DirectMembershipEvidence(artist_id="artist:two", genre_id="genre:b", facet="musicbrainz_tag", value=10, evidence_ref="direct:two:b"),
        ),
        artist_pairs=(
            ArtistPairEvidence(left_artist_id="artist:one", right_artist_id="artist:two", listener_day_support=support, supporting_windows=2, evidence_refs=("listenbrainz:fixture",)),
        ),
        hierarchy=(GenreHierarchyEdge(child_genre_id="genre:a", parent_genre_id="genre:b", evidence_ref="taxonomy:a:b"),) if hierarchy else (),
    )
...
class OneHopMembershipCandidateTests(unittest.TestCase):
    def test_seals_deterministic_public_only_memberships(self) -> None:
        ...
        self.assertEqual({(item.artist_id, item.genre_id) for item in first.memberships}, {("artist:one", "genre:b"), ("artist:two", "genre:a")})
        self.assertTrue(all(path.edge_type == "listenbrainz_privacy_safe_co_listen" for item in first.memberships for path in item.paths))
    ...
    def test_abstains_for_weak_edges_and_missing_taxonomy_anchor(self) -> None:
        ...
        self.assertEqual(dict((item.reason, item.count) for item in weak.coverage.abstentions)["edge_support"], 2)
        self.assertEqual(missing.coverage.accepted_membership_count, 0)
        self.assertEqual(dict((item.reason, item.count) for item in missing.coverage.abstentions)["target_taxonomy_distance"], 2)
