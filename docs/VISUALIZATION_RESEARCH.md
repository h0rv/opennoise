...
Use one semantic-zoom graph. The overview presents a small number of map
communities or umbrellas. Zooming progressively reveals the public taxonomy and
genre graph. A selected genre shows its evidence, nearby genres, artists,
albums, and tracks.

Hackerverse is the useful reference for this interaction model: a map-like
camera, persistent context through levels of detail, sparse labels, and stable
spatial sampling. Musix adapts those product ideas, not its terrain renderer,
embedding, tile format, or data pipeline. At this size, one versioned graph
artifact is simpler than tiles or a multi-client data protocol.
...
production pan or semantic zoom. Canvas, WebGL, workers, binary tiles, and
edge bundling are deferred until measured need proves that Cytoscape cannot meet
the target interaction budget.
