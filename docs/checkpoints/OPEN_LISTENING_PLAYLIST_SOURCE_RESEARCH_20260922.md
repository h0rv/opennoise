...
| Last.fm 1K | The creator-hosted primary page for the timestamped 1K rows was unavailable at this check, so no field schema is accepted into custody from a secondary description. Its historical source URL is [Last.fm 1K](https://www.dtic.upf.edu/~ocelma/MusicRecommendationDataset/lastfm-1K.html). | No verified current primary acquisition path or exact-ID bridge was established in this check. | Do not acquire from mirrors or use it. If the original source returns under an approved terms review, inspect the primary schema afresh; it would still be listening evidence, not playlist curation or genre gold. |
