...
| MSD and Last.fm offline files | The three required MSD SQLite inputs remain absent. Separately, a verified local-only Last.fm 360K archive and its 100,000-row prefix diagnostic are retained under ignored cache with source receipt and checksums; see [the checkpoint](LASTFM_360K_SOURCE_FEASIBILITY_20260922.md). | No graph, membership, model, or public use. The 360K result is not MSD evidence. | The 360K diagnostic reports only aggregate exact-ID coverage and a privacy floor count. Its source-order prefix and first-ten-artist-per-user bound make it unsuitable as full-dataset coverage or a similarity result. |
...
local-only and intentionally not a model or evaluation input. Fourth, no
