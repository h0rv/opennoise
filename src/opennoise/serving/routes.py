...
from opennoise.db import AsyncDatabase
...
from opennoise.serving.exploration import (
    CatalogLens,
    GenreDetail,
...
from opennoise.serving.local.musicbrainz_peer_store import (
...
if TYPE_CHECKING:
    from opennoise.models.historical_signal import HistoricalSignalHierarchyNode
    from opennoise.serving.open.static_map import StaticOpenMap
from opennoise.serving.map.production_store import (
...
class MapController(Controller):
    ...
    async def historical_signal_map_data(
        ...
        if not historical_signal_map.configured:
            raise ServiceUnavailableException(
                detail=(
                    "historical signal map is disabled; configure "
                    "OPENNOISE_HISTORICAL_SIGNAL_MAP_PATH"
                )
            )
        try:
...
async def workspace_context(  # noqa: C901
    ...
    *,
    ...
    if production_map.configured and layout == "default":
        ...
        except ProductionMapStoreError:
            # The HTML shell stays usable while an optional production artifact is rebuilt.
            # The explicit JSON route below remains strict and reports the 503.
            production_graph = None
