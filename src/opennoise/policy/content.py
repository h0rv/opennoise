"""Reject audio and stream payloads before OpenNoise reads or stores them."""
...
def require_metadata_media_type(media_type: str) -> None:
    ...
    if normalized in _FORBIDDEN_MEDIA_TYPES or normalized.startswith(_FORBIDDEN_MEDIA_PREFIXES):
        raise AudioContentRejectedError(
            "OpenNoise accepts metadata artifacts only, never media bytes"
        )
...
def require_metadata_path(path: str | PurePath) -> None:
    ...
    if suffix in _FORBIDDEN_SUFFIXES:
        raise AudioContentRejectedError(f"OpenNoise rejected media payload path {str(path)!r}")
...
def require_metadata_prefix(prefix: bytes) -> None:
    ...
    if (
        ...
        raise AudioContentRejectedError("OpenNoise rejected an audio or media byte signature")
