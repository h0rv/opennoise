...
def evaluate_calibrated_replica(
    *,
    ...
    name_scores = name_tfidf_cosine(names)
    ...
    for cutoff in (4, 8, 16):
        ...
        for artist_weight in (0.0, 0.25, 0.5, 0.75, 1.0):
            for k in (10, 20):
                blended = {
                    pair: artist_weight * cached[cutoff].get(pair, 0.0)
                    + (1.0 - artist_weight) * name_scores.get(pair, 0.0)
                    for pair in cached[cutoff] | name_scores
                }
                results.append(
                    ReplicaGridResult(
                        ...
                        train_recall_at_k=_recall(
                            train_ids, _coordinate_neighbors(train_ids, coordinates, k), blended, k
                        ),
                    )
                )
    selected = max(
    ...
    chosen = {
        pair: selected.artist_weight * cached[selected.maximum_artist_genre_degree].get(pair, 0.0)
        + (1.0 - selected.artist_weight) * name_scores.get(pair, 0.0)
        for pair in cached[selected.maximum_artist_genre_degree] | name_scores
    }
