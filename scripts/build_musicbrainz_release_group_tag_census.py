"""Write a create-only aggregate census of native MusicBrainz release-group tags."""

from pathlib import Path

from opennoise.ingest.musicbrainz.entity_genre_observation import write_local_report_once
from opennoise.ingest.musicbrainz.release_group_tag_census import build_release_group_tag_census
...
def main() -> int:
    """Run one bounded local census against the retained pinned source files."""
    archive = Path(
    ...
    write_local_report_once(
        ...
        output=Path(".cache/musicbrainz-release-group-tag-census-v1/report.json"),
