"""Render a local-only static HTML preview of MusicBrainz credit metadata."""

from __future__ import annotations

import argparse
from pathlib import Path

from opennoise.deployment.musicbrainz_credit_artist_detail_local_html import (
    export_local_musicbrainz_credit_artist_detail_html_preview,
)


def main() -> int:
    """Write a fresh local preview directory and never choose ``dist``."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--credit-metadata", required=True, type=Path)
    parser.add_argument("--approval", required=True, type=Path)
    parser.add_argument("--static-discovery", required=True, type=Path)
    parser.add_argument("--static-artist-id", required=True)
    parser.add_argument("--local-root", required=True, type=Path)
    parser.add_argument("--output-directory", required=True, type=Path)
    arguments = parser.parse_args()
    preview = export_local_musicbrainz_credit_artist_detail_html_preview(
        credit_metadata=arguments.credit_metadata,
        approval=arguments.approval,
        static_discovery=arguments.static_discovery,
        static_artist_id=arguments.static_artist_id,
        local_root=arguments.local_root,
        output_directory=arguments.output_directory,
    )
    print(preview.html_path)  # noqa: T201
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
